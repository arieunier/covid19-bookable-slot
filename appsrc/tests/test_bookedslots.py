...
class TestCases(unittest.TestCase):
    ...
    def test_access_unauthenticated(self):
        ...
        result, code = appsrc.tests.utils.HTTP_GET(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, {})
        self.assertEqual(code, 500)
        # tries to get the booking with the code, must work
        result, code = appsrc.tests.utils.HTTP_GET(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, ujson.dumps({'confirmationCode':bookedSlotCode}))
        self.assertEqual(code, 200) 
        
        
        # updates the booking without the code, must fail
        result, code = appsrc.tests.utils.HTTP_PUT(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, ujson.dumps({'firstname':'newfname', }))
        self.assertEqual(code, 500)

        # updates the booking without  code but with wrong data, must fail
        result, code = appsrc.tests.utils.HTTP_PUT(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, ujson.dumps({'firstname':'', 'confirmationCode':bookedSlotCode}))
        self.assertEqual(code, 500)

        # updates the booking without  code but with correct data, must work
        result, code = appsrc.tests.utils.HTTP_PUT(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, ujson.dumps({'firstname':'new name', 'confirmationCode':bookedSlotCode}))
        self.assertEqual(code, 200)

        # updating nb of person  but more capacity
        result, code = appsrc.tests.utils.HTTP_PUT(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, ujson.dumps({'numberOfPerson':3, 'confirmationCode':bookedSlotCode}))
        self.assertEqual(code, 500)
        ...
        result, code = appsrc.tests.utils.HTTP_PUT(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, ujson.dumps({'refBookableSlotId':secondBookableSlotId, 'confirmationCode':bookedSlotCode}))
        self.assertEqual(code, 500)
        ...
        # deletes the slot
        result, code = appsrc.tests.utils.HTTP_DEL(variables.DEFAULT_API_URL + '/bookedslots/' + bookedSlotId, {}, {}, ujson.dumps({'confirmationCode':bookedSlotCode}))
        self.assertEqual(code, 200)
